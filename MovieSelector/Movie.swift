...
public struct Movie {
    ...
    private static func checkForImageData (withMovieOjbect movie:Movie) -> String? {
    ...
    public static func  getImage (forCell cell:AnyObject, withMovieObject movie:Movie) {
        if let imagePath = checkForImageData(withMovieOjbect: movie) { // image already downloaded
